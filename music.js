...
const opts = {
    ...
    ySearch(searchQuery, opts, function (err, results) {
        if (err) {
            msg.channel.send(`Désolé, je ne trouve pas cette vidéo.`);
            return console.log(err);
        }
        for (var y = 0; results[y].kind === 'youtube#channel'; y++);
        ytdl.getInfo(results[y].link, function (err, song) {
            if (err) {
                msg.channel.send(`Désolé, je n'ai pas trouver cette chanson.`);
                return console.log(err);
            }
    ...
    */
    song(msg, guild, url) {
        ytdl.getInfo(url, (err, song) => {
            if (err) {
                console.log(err);
                msg.channel.send(`Désolé, je ne peut pas ajouter cette chanson a la fils d'attente.`);
                return;
            }
            const author  = msg.author.username + '#' + msg.author.discriminator;
            console.log(song);
    ...
    for (let guildId in guilds) {
        ...
        if (guild.inactivityTimer <= 0) {
            ...
            guild.musicChannel.send(
                `:no_entry_sign: Quitter le cnal vocal en raison de l'inactivité.`);
